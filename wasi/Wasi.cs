...
public static partial class wasi_unstable
{
		...
		)
    {
        ...
        var path = util.from_utf8(sg_wasm.__mem + addr_path, path_len);
        ...
        try
        {
            ...
            var fd = get_new_fd();
            _files[fd] = pair;
            Marshal.WriteInt32(sg_wasm.__mem + addr_fd, fd);
    ...
    public static int fd_prestat_dir_name(int fd, int addr_path, int len)
    {
        ...
        {
            case 3:
                Marshal.WriteByte(sg_wasm.__mem + addr_path, 46); // 46 is ascii for .
    ...
    public static int fd_prestat_get(int fd, int addr)
    {
        ...
        switch (fd)
        {
            case 3:
                ...
                Marshal.StructureToPtr(st, sg_wasm.__mem + addr, false);
                return __WASI_ESUCCESS;
            default:
                return __WASI_EBADF;
        }
    ...
    public static int environ_sizes_get(int addr_environ_count, int addr_environ_buf_size)
    {
        if (__environ != null)
        {
            Marshal.WriteInt32(sg_wasm.__mem + addr_environ_count, __environ.Length);
            int len = 0;
            foreach (var ba in __environ)
            {
                len += ba.Length;
            }
            Marshal.WriteInt32(sg_wasm.__mem + addr_environ_buf_size, len);
        }
        else
        {
            Marshal.WriteInt32(sg_wasm.__mem + addr_environ_count, 0);
            Marshal.WriteInt32(sg_wasm.__mem + addr_environ_buf_size, 0);
        }
        return __WASI_ESUCCESS;
    }
    public static int environ_get(int addr_argv, int addr_argv_buf)
    {
        ...
        {
            Marshal.WriteInt32(sg_wasm.__mem + addr_argv + i * 4, addr_argv_buf + sofar);
            var ba = __environ[i];
            Marshal.Copy(ba, 0, sg_wasm.__mem + addr_argv_buf + sofar, ba.Length);
            sofar += ba.Length;
        }
        return __WASI_ESUCCESS;
    }
    ...
    public static int args_sizes_get(int addr_argc, int addr_argv_buf_size)
    {
        if (__args != null)
        {
            Marshal.WriteInt32(sg_wasm.__mem + addr_argc, __args.Length);
            int len = 0;
            foreach (var ba in __args)
            {
                len += ba.Length;
            }
            Marshal.WriteInt32(sg_wasm.__mem + addr_argv_buf_size, len);
        }
        else
        {
            Marshal.WriteInt32(sg_wasm.__mem + addr_argc, 0);
            Marshal.WriteInt32(sg_wasm.__mem + addr_argv_buf_size, 0);
        }
        return __WASI_ESUCCESS;
    }
            ...
            case __WASI_CLOCK_REALTIME:
                {
                    ...
                    var ns = ms * 1000 * 1000;
                    Marshal.WriteInt64(sg_wasm.__mem + addr_result, ns);
                    return __WASI_ESUCCESS;
                }
    ...
    public static int fd_read(int fd, int addr_iovecs, int iovecs_len, int addr_nread)
    {
        ...
        unsafe
        {
            a_iovecs = new Span<__wasi_iovec_t>((sg_wasm.__mem + addr_iovecs).ToPointer(), iovecs_len);
        }
        ...
        for (int i=0; i<iovecs_len; i++)
        {
            ...
            unsafe
            {
                dest = new Span<byte>((sg_wasm.__mem + (int) addr).ToPointer(), (int) len);
            }
        ...
        Marshal.WriteInt32(sg_wasm.__mem + addr_nread, total_len);
    ...
    public static int fd_write(int fd, int addr_iovecs, int iovecs_len, int addr_nwritten)
    {
        ...
        unsafe
        {
            a_iovecs = new Span<__wasi_ciovec_t>((sg_wasm.__mem + addr_iovecs).ToPointer(), iovecs_len);
        }
        ...
        for (int i=0; i<iovecs_len; i++)
        {
            ...
            unsafe
            {
                src = new Span<byte>((sg_wasm.__mem + (int) addr).ToPointer(), (int) len);
            }
            strm.Write(src);
            total_len += (int) len;
        }

        Marshal.WriteInt32(sg_wasm.__mem + addr_nwritten, total_len);
    ...
    static void write_u64(int addr, ulong v)
    {
        ...
        Marshal.Copy(ba, 0, sg_wasm.__mem + addr, ba.Length);
    }
    ...
    {
        var old_path = util.from_utf8(sg_wasm.__mem + addr_old_path, old_path_len);
        var new_path = util.from_utf8(sg_wasm.__mem + addr_new_path, new_path_len);
        throw new NotImplementedException();
    }
